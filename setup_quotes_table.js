#!/usr/bin/env node
/**
 * 创建「金句摘抄」多维表格并补齐字段（幂等，可重复执行）
 *   Base：人生研究学院（INSPIRE_BASE）
 *   字段：内容(文本) / 分类(文本，允许自由新增分类) / 记录时间(日期时间，存本地创建时间)
 *
 * 用法：在 workbench 目录下执行
 *   node setup_quotes_table.js
 * 需要 .env.local 或环境变量提供 FEISHU_APP_ID / FEISHU_APP_SECRET
 */
const fs = require('fs');
...
const BASE_TOKEN = env.INSPIRE_BASE_TOKEN || 'ARCcbggiUaFqESsV7pRcin8CnUb';
...
(async () => {
  ...
  const want = [
    { name: '内容', type: 1 },
    { name: '分类', type: 1 },
    { name: '记录时间', type: 5, property: { auto_fill: false, date_formatter: 'yyyy-MM-dd HH:mm' } },
  ];
  for (const w of want) {
    ...
    const r = await req('POST', PRE + '/tables/' + q.table_id + '/fields', body, token);
